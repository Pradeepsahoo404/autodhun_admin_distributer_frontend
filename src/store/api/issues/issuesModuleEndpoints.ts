...
type IssuesModuleBuilder = EndpointBuilder<
  BaseQueryFn<string | FetchArgs, unknown, FetchBaseQueryError>,
  string,
  'api'
>;

export interface IssuesModuleEndpointConfig<T> {
  tag: string;
  basePath: string;
  exportPath?: string;
}

export function issuesModuleEndpoints<T>(builder: IssuesModuleBuilder, config: IssuesModuleEndpointConfig<T>) {
  ...
  return {
    ...
    updateOwnership: builder.mutation<ApiSuccess<T>, { id: string; body: { ownership: 'yes' | 'no' } }>({
      query: ({ id, body }) => ({ url: `${basePath}/${id}/ownership`, method: 'PATCH', body }),
      invalidatesTags: [tag],
    }),
