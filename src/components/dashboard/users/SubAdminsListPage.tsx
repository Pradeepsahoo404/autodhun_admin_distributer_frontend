...
export function SubAdminsListPage() {
  const router = useRouter();
  const { user: currentUser } = useAppSelector((s) => s.auth);
  const { canCreate, canUpdate, canDelete } = usePermission('sub-admins');
  ...
  const { data, isLoading, isFetching } = useGetSubAdminsQuery(queryParams);
  ...
  const handleStatusToggle = async (user: User, checked: boolean) => {
    ...
    try {
      ...
      toast.success(checked ? 'Sub Admin access enabled' : 'Sub Admin deactivated and signed out if online');
    } catch (error) {
  ...
  const emptyMessage =
    statusFilter === 'active'
      ? 'No active Sub Admins found.'
      : statusFilter === 'inactive'
        ? 'No inactive Sub Admins found.'
        : 'No Sub Admins found.';
    ...
    <div className={cn(DASHBOARD_PAGE, 'space-y-8')}>
      <DashboardPageHeader
        ...
        action={
          canCreate ? (
            <Button
              onClick={() => router.push(ROUTES.SUB_ADMINS_CREATE)}
      ...
      <Card className={legalModuleCardClass}>
        <CardHeader className={legalModuleCardHeaderClass}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <CardTitle className="text-white">Sub Admins</CardTitle>
        ...
        <CardContent className="pt-5">
          {isLoading ? (
            <p className="py-10 text-center text-neutral-500">Loading Sub Admins...</p>
          ) : users.length === 0 ? (
                ...
                <table className={cn(dashboardTableClass, 'min-w-[760px]')}>
                  ...
                  <tbody className={dashboardTableBodyClass}>
                    {users.map((user) => {
                      ...
                      const showResendInvite = canCreate && isInvitePending(user);
                        ...
                        <tr key={user._id} className={dashboardTableRowClass}>
                          ...
                          <td className={dashboardTableCellActions}>
                            <TableRowActions
                              ...
                              onEdit={() => router.push(`${ROUTES.SUB_ADMINS}/${user._id}/edit`)}
