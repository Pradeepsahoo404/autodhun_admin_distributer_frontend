  ...
  return (
    <div className={cn(DASHBOARD_PAGE, 'space-y-8')}>
      <DashboardPageHeader
        title="Content ID"
        description={
          isElevated            ? 'Review all content ID entries, manage in-progress entries, and activate or deactivate them'
        ...
        <CardHeader className={legalModuleCardHeaderClass}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <CardTitle className="text-white">
              {isElevated ? 'All content ID entries' : 'My content ID entries'}
            </CardTitle>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center gap-4 py-12 text-center">
              <p className="text-neutral-500">
                {isElevated ? 'No content ID entries found.' : 'You have not created any content ID entries yet.'}
              </p>
              {!isElevated && canCreate ? (
                <Button
                  onClick={() => setCreateOpen(true)}
