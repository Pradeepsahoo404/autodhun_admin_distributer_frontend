  ...
  return (
    <div className={cn(DASHBOARD_PAGE, 'space-y-8')}>
      <DashboardPageHeader
        title="OAC"
        description={
          isElevated            ? 'Review all official artist channel entries, manage in-progress entries, and activate or deactivate them'
        ...
        <CardHeader className={legalModuleCardHeaderClass}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <CardTitle className="text-white">
              {isElevated ? 'All OAC entries' : 'My OAC entries'}
            </CardTitle>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center gap-4 py-12 text-center">
              <p className="text-neutral-500">
                {isElevated ? 'No OAC entries found.' : 'You have not created any OAC entries yet.'}
              </p>
              {!isElevated && canCreate ? (
                <Button
                  onClick={() => setCreateOpen(true)}
