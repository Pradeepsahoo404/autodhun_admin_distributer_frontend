...
import { CreateClaimReleaseDialog } from '@/components/dashboard/facebook-claim-release/CreateClaimReleaseDialog';
import { EditClaimReleaseDialog } from '@/components/dashboard/facebook-claim-release/EditClaimReleaseDialog';
import { DeleteClaimReleaseDialog } from '@/components/dashboard/facebook-claim-release/DeleteClaimReleaseDialog';
import { FacebookLinkCell } from '@/components/dashboard/facebook-claim-release/FacebookLinkCell';
...
export default function FacebookClaimReleasePage() {
  ...
  const [editItem, setEditItem] = useState<FacebookClaimRelease | null>(null);
  const [deleteItem, setDeleteItem] = useState<FacebookClaimRelease | null>(null);
  ...
  const { data, isLoading, isFetching } = useGetFacebookClaimReleasesQuery(queryParams);
  const [updateStatus] = useUpdateFacebookClaimReleaseStatusMutation();
  const [deleteClaim, { isLoading: deleting }] = useDeleteFacebookClaimReleaseMutation();
  const [exportCsv, { isLoading: exporting }] = useExportFacebookClaimReleasesMutation();

  const items = data?.data ?? [];
  const meta = data?.meta as PaginatedMeta | undefined;

  const handleStatusChange = async (item: FacebookClaimRelease, status: 'active' | 'inactive') => {
  ...
  const handleExport = async () => {
    try {
      ...
      link.href = url;
      link.download = `facebook-claim-releases-${new Date().toISOString().slice(0, 10)}.csv`;
                    ...
                    {items.map((item) => (
                        <tr key={item._id} className={legalEntryRowClass(dashboardTableRowClass, item._id, highlightId)}>
                          ...
                          <td className="px-4 py-4">
                            <FacebookLinkCell href={item.facebookPageLink} />
                          </td>
                          <td className="px-4 py-4">
