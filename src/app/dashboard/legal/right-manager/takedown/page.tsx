...
import { CreateTakedownDialog } from '@/components/dashboard/takedown/CreateTakedownDialog';
import { EditTakedownDialog } from '@/components/dashboard/takedown/EditTakedownDialog';
import { DeleteTakedownDialog } from '@/components/dashboard/takedown/DeleteTakedownDialog';
import { TakedownLinkCell } from '@/components/dashboard/takedown/TakedownLinkCell';
import { formatDateTime } from '@/lib/formatDateTime';
import { useLegalEntryHighlight, legalEntryRowClass } from '@/hooks/useLegalEntryHighlight';
import { cn } from '@/lib/utils';
import type { PaginatedMeta, Takedown } from '@/types';
...
export default function TakedownPage() {
  ...
  const { canCreate, canUpdate, canDelete } = usePermission('takedown');
  ...
  const [editItem, setEditItem] = useState<Takedown | null>(null);
  const [deleteItem, setDeleteItem] = useState<Takedown | null>(null);
  ...
  const { data, isLoading, isFetching } = useGetTakedownEntriesQuery(queryParams);
  const [updateStatus] = useUpdateTakedownEntryStatusMutation();
  const [deleteEntry, { isLoading: deleting }] = useDeleteTakedownEntryMutation();
  const [exportCsv, { isLoading: exporting }] = useExportTakedownEntriesMutation();

  const items = data?.data ?? [];
  const meta = data?.meta as PaginatedMeta | undefined;

  const handleStatusChange = async (item: Takedown, status: 'active' | 'inactive') => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success('Takedown entry deleted');
      setDeleteItem(null);
    } catch (error) {
      toast.error(getApiErrorMessage(error));
    }
  ...
  const handleExport = async () => {
    try {
      ...
      link.href = url;
      link.download = `takedown-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className={cn(DASHBOARD_PAGE, 'space-y-8')}>
      <DashboardPageHeader
        title="Takedown"
        description={
          isElevated            ? 'Review all takedown entries, manage in-progress entries, and activate or deactivate them'
            : 'Create and manage your takedown requests'
        }
        action={
      ...
      <Card className={legalModuleCardClass}>
        <CardHeader className={legalModuleCardHeaderClass}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <CardTitle className="text-white">
              {isElevated ? 'All takedown entries' : 'My takedown entries'}
            </CardTitle>
        ...
        <CardContent className="pt-5">
          {isLoading ? (
            <p className="py-10 text-center text-neutral-500">Loading takedown entries...</p>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center gap-4 py-12 text-center">
              <p className="text-neutral-500">
                {isElevated ? 'No takedown entries found.' : 'You have not created any takedown entries yet.'}
              </p>
              {!isElevated && canCreate ? (
                <Button
                  onClick={() => setCreateOpen(true)}
                  ...
                  <tbody className={dashboardTableBodyClass}>
                    {items.map((item) => (
                        <tr key={item._id} className={legalEntryRowClass(dashboardTableRowClass, item._id, highlightId)}>
                          ...
                          <td className="px-4 py-4">
                            <TakedownLinkCell href={item.songLink} />
                          </td>
                          <td className="px-4 py-4">
      ...
      <CreateTakedownDialog open={createOpen} onClose={() => setCreateOpen(false)} />
      <EditTakedownDialog open={Boolean(editItem)} item={editItem} onClose={() => setEditItem(null)} />
      <DeleteTakedownDialog
