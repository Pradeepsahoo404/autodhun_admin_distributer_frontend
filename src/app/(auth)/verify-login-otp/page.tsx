...
import { useVerifyLoginOtpMutation, useResendOtpMutation, useGetSidebarQuery } from '@/store/api';
...
export default function VerifyLoginOtpPage() {
  ...
  const [verify, { isLoading }] = useVerifyLoginOtpMutation();
  const [resend, { isLoading: resending }] = useResendOtpMutation();
  const { refetch: refetchSidebar } = useGetSidebarQuery(undefined, { skip: true });
  ...
  const handleVerify = useCallback(
    ...
    [dispatch, email, isLoading, otp, refetchSidebar, router, verify],
  );

  const handleResend = async () => {
    try {
      const response = await resend({ email, purpose: OTP_PURPOSE.LOGIN }).unwrap();
